...
async function switchToAnalysis(doAnalysis) {
    ...
    if (doAnalysis) {
        ...
        if (typeof switchButton !== 'undefined' && switchButton) {
...
async function playAgain() {
    ...
    if (board != null && !analysisMode) {
        ...
        document.getElementById("newGameSmiley").src = 'JSMinesweeper/resources/images/face.svg';
...
async function newBoardFromString(data, inflate, analyse) {
    ...
    if (!board || typeof board.getHashValue !== 'function' || typeof board.width === 'undefined' || typeof board.height === 'undefined') {
        console.error('[newBoardFromString] Board object is malformed:', board);
        alert('Failed to load board: Malformed board object.');
        return;
    }
...
function doToggleFlag() {
    ...
    if (leftClickFlag) {
        document.getElementById("leftClickFlag").src = 'JSMinesweeper/resources/images/flaggedWrong_thin.png';
        leftClickFlag = false;
    } else {
        document.getElementById("leftClickFlag").src = 'JSMinesweeper/resources/images/flagged.png';
        leftClickFlag = true;
    }
...
async function changeTileSize(analyse) {
    ...
    // Add defensive check for board existence and valid dimensions
    if (!board || typeof board.width === 'undefined' || typeof board.height === 'undefined') {
        console.error('Cannot resize canvas: Board is not properly initialized');
        return;
    }
...
function browserResized() {
    // Defensive: Only proceed if the game board and canvases exist
    var canvas = document.getElementById('myCanvas');
    var minesLeft = document.getElementById('myMinesLeft');
    if (!canvas || !minesLeft) return;

    // Guard: Only proceed if board is initialized and has width/height
    if (typeof board === 'undefined' || board === null) return;
    if (typeof board.width === 'undefined' || typeof board.height === 'undefined') return;
...
async function sendActionsMessage(message) {
    ...
    if (board.isGameover()) {
        ...
        if (reply.header.status == "won") {
            ...
            document.getElementById("newGameSmiley").src = 'JSMinesweeper/resources/images/face_win.svg';
        } else {
            efficiency = (100 * solved3BV / actionsMade).toFixed(2) + "%";
            document.getElementById("newGameSmiley").src = 'JSMinesweeper/resources/images/face_lose.svg';
        }
...
function load_images() {
    ...
    for (let i = 0; i <= 8; i++) {
        const file_path = "JSMinesweeper/resources/images/" + i.toString() + ".png";
        images.push(load_image(file_path));
        const led_path = "JSMinesweeper/resources/images/led" + i.toString() + ".svg";
        led_images.push(load_image(led_path));
    }

    led_images.push(load_image("JSMinesweeper/resources/images/led9.svg"));
    led_images.push(load_image("JSMinesweeper/resources/images/led-.svg"));
    images.push(load_image("JSMinesweeper/resources/images/bomb.png"));
    images.push(load_image("JSMinesweeper/resources/images/facingDown.png"));
    images.push(load_image("JSMinesweeper/resources/images/flagged.png"));
    images.push(load_image("JSMinesweeper/resources/images/flaggedWrong.png"));
    images.push(load_image("JSMinesweeper/resources/images/exploded.png"));
    images.push(load_image("JSMinesweeper/resources/images/skull.png"));
    images.push(load_image("JSMinesweeper/resources/images/start.png"));
